        ...
        if (currentDb > 75 && nowTime - lastNotificationTimeRef.current > NOTIFICATION_COOLDOWN) {
         ...
         await Notifications.scheduleNotificationAsync({
          content: {
            ...
            sound: true,
            priority: Notifications.AndroidNotificationPriority.MAX,
            // تم حذف كائن android من هنا لأن النوع NotificationContentInput لا يدعمه مباشرة
          },
          trigger: {
            channelId: 'noise_alerts', // نضع المعرف هنا لربطه بالإعدادات التي قمتِ بتعريفها
          },
        });
        }
...
📊 الضجيج: ${data.bestRoute.avgDb || data.bestRoute.noiseLevel} dB
⏰ رُصد في: ${formatTime(data.bestRoute.timestamp || new Date().toISOString())}
📏 المسافة: ${data.bestRoute.distance ? data.bestRoute.distance.toFixed(1) + ' كم' : 'قريب جداً'}`,
          [{ text: "حسناً" }]
        );
      } else {
  ...
  async function registerForPushNotifications() {
    ...
    // ✅ FIX 3: إنشاء الـ channel مرة وحدة هون
  ...
  useEffect(() => {
    ...
    socket.current.on('connect', async () => {
      ...
      // ✅ FIX 3: استخدام الـ token المحفوظ مسبقاً
      socket.current.emit('register-device', { deviceName: Platform.OS, localIP: ip, pushToken: pushTokenRef.current });
    });
    ...
    // ✅ FIX 3: تسجيل الإشعارات وحفظ الـ token مرة وحدة عند البدء
    (async () => {
...
>
          ...
          <TouchableOpacity style={[styles.aiButton, { backgroundColor: themeColors.accent, shadowColor: themeColors.accent }]} onPress={activateMagicWand} disabled={isAiLoading}>
            {isAiLoading ? <ActivityIndicator color={isDarkMode ? '#000' : '#fff'} /> : <MaterialCommunityIcons name="auto-fix" size={30} color={isDarkMode ? '#000' : '#fff'} />}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  aiButton: { position: 'absolute', bottom: 40, right: 25, width: 70, height: 70, borderRadius: 35, justifyContent: 'center', alignItems: 'center', elevation: 15 },
